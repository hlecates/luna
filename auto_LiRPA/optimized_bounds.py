...
default_optimize_bound_args = {
    ...
    'debug_intermediate_bounds': False,  # Print intermediate layer bounds per iteration.
...
def _print_intermediate_bounds(
        self: 'BoundedModule', iteration: int, total_iterations: int,
        optimizable_activations: list):
    """Print intermediate layer bounds (pre-activation) for debugging."""
    print(f"[DEBUG Iter {iteration}/{total_iterations}] "
          f"===== INTERMEDIATE LAYER BOUNDS (pre-activation) =====")
    for node in optimizable_activations:
        # Get pre-activation bounds (input to the activation node)
        if node.inputs[0].lower is None or node.inputs[0].upper is None:
            continue
        lower = node.inputs[0].lower
        upper = node.inputs[0].upper
        # Flatten for per-neuron printing
        lower_flat = lower.flatten()
        upper_flat = upper.flatten()
        num_neurons = lower_flat.numel()
        # Get the input node name (pre-activation layer)
        input_node = node.inputs[0]
        print(f"[DEBUG Iter {iteration}] {node.__class__.__name__} {node.name} "
              f"input ({input_node.name}): {num_neurons} neurons")
        # Print per-neuron bounds
        active_count = 0
        inactive_count = 0
        unstable_count = 0
        for i in range(num_neurons):
            l_val = lower_flat[i].item()
            u_val = upper_flat[i].item()
            # Determine neuron status
            if l_val >= 0:
                status = "active"
                active_count += 1
            elif u_val <= 0:
                ...
                status = "unstable"
                unstable_count += 1
            print(f"  neuron {i}: L={l_val:.4f}, U={u_val:.4f} [{status}]")
        print(f"  Summary: active={active_count}, inactive={inactive_count}, "
              f"unstable={unstable_count}")
    print(f"[DEBUG Iter {iteration}/{total_iterations}] "
          f"===== END INTERMEDIATE BOUNDS =====")
    # Print alpha values
    print(f"[DEBUG Iter {iteration}/{total_iterations}] "
          f"===== ALPHA VALUES =====")
    for node in optimizable_activations:
        if not hasattr(node, 'alpha') or not node.alpha:
            continue
        for start_node_name, alpha_tensor in node.alpha.items():
            # alpha shape: [2, spec, batch, *relu_shape]
            # Index 0 is for lower bound, index 1 is for upper bound
            # We typically care about the lower bound alpha (index 0)
            # Flatten the alpha values for this node
            alpha_flat = alpha_tensor[0].flatten()  # Lower bound alphas
            num_alphas = alpha_flat.numel()
            print(f"[DEBUG Iter {iteration}] Alpha values for node {node.name} "
                  f"(start_node: {start_node_name}):")
            for i in range(num_alphas):
                print(f"  alpha[{i}] = {alpha_flat[i].item():.4f}")
    print(f"[DEBUG Iter {iteration}/{total_iterations}] "
          f"===== END ALPHA VALUES =====")
...
def _get_optimized_bounds(
    ...
    debug_intermediate_bounds = opts.get('debug_intermediate_bounds', False)
    ...
    for i in range(iteration):
        ...
        # Debug: print intermediate layer bounds for each iteration
        if debug_intermediate_bounds:
            _print_intermediate_bounds(
                self, i + 1, iteration, optimizable_activations)
