...
int main(int argc, char** argv) {
    ...
    // Mode selection: default standard CROWN (layer-by-layer intermediates via CROWN). Pass "--ibp" to use CROWN-IBP.
    bool useStandardCROWN = true;
    for (int i = 1; i < argc; ++i) {
        std::string arg(argv[i]);
        if (arg == "--ibp") useStandardCROWN = false;
    }
    std::cout << "Mode: " << (useStandardCROWN ? "Standard CROWN" : "CROWN-IBP") << std::endl;
    ...
    try {
        ...
        NLR::CROWNAnalysis crownAnalysis(model, useStandardCROWN);
        crownAnalysis.run();
